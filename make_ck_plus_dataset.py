...
class CKPlusCondenser(object):
    def __init__(self, original_dataset_path, condensed_dataset_path):
        ...
        self.image_path = os.path.join(condensed_dataset_path,'cohn-kanade-images')
        self.label_path = os.path.join(condensed_dataset_path,'Emotion_labels')
...
class CKPlusFaceCropper(object):
    ...
    def crop_and_align_all_faces(self, path):
        ...
        for image_file_path in all_image_paths:
            ...
            I, success_flag = self.process_single_image(image_file_path,output_img_size)
            if not success_flag:
        ...
        missed_faces_file_path = os.path.join(self.input_path,'missed_faces.txt')
    ...
    def detect_crop_face(self, I):
        success_flag = False
        face_detector = FaceDetector(scale_factor=1.3, min_neighbors=5,min_size_scalar=0.5, max_size_scalar=0.8)
        faces = face_detector.detect_faces(I)

        # If face was not detected:
        if len(faces) == 0:
            # Try with more lenient conditions
            face_detector = FaceDetector(scale_factor=1.3,min_neighbors=3,min_size_scalar=0.5,max_size_scalar=0.8)
            faces = face_detector.detect_faces(I)
            if len(faces) == 0:
...
class CKPlusNumpyFileGenerator(object):
    ...
    def make_data_label_mats(self, all_images_path,all_labels_path, num_samples):
            # Initialize the data of interest
            image_shape = (96, 96, 1)
            X = numpy.zeros((num_samples, image_shape[2],image_shape[0], image_shape[1]), dtype='uint8')
            ...
            for i, subj in enumerate(subj_list):
                ...
                for j, seq in enumerate(seq_list):
                    ...
                    images = self.read_images(all_images_path, subj, seq,image_shape)
                    label = self.read_label(all_labels_path, subj, seq)
                    label_vec = numpy.array([label])
    ...
    def read_images(self, all_images_path, subj, seq, image_shape):
        ...
        images = numpy.zeros((num_images, image_shape[2], image_shape[0], image_shape[1]))
        for i, image_file in enumerate(image_files):
            print('Reading image:', os.path.join(image_file_path, image_file))
    ...
    def make_folds(self, subjs, num_folds=10):
        ...
        print ('Number of samples/fold: %s' % numpy.histogram(folds,bins=10)[0])
...
class FaceDetector(object):
    def __init__(self, scale_factor=1.3, min_neighbors=5,min_size_scalar=0.25, max_size_scalar=0.75):
    ...
    def detect_faces(self, I):
        ...
        faces = self.detector.detectMultiScale(I, self.scale_factor,self.min_neighbors, 0,min_size,max_size)
        return faces
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ip', '--input_path', dest='input_path',
                        help='Path specifying location of downloaded ''CK+ files.')
    parser.add_argument('-sp', '--save_path', dest='save_path',
                        default='./CK_PLUS_HERE',
                        help='Path specifying where to save \the pre-processed dataset and the \output (.npy) files.')
